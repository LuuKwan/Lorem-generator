...
const articles = document.querySelector(`.lorem-text`)

form.addEventListener(`submit`,(item)=>{
 item.preventDefault() // prevent form automatically reset
 let paragraphs =[]
 const quantity = parseInt(amount.value);// convert to int value
 if(isNaN(quantity) || quantity <= 0 || quantity >= text.length){
  articles.innerHTML= ``
  alert(`number of paragraph should be greater than 0 and less than 9`)
 }
 else{
  for(let i = 0; i < quantity;++i){
   let randomIndex = Math.floor(Math.random() * text.length)
   paragraphs += `<p>${text[randomIndex]}</p>\n` 
   
  }
 }
 articles.innerHTML = paragraphs
})